    ...
    for (x, y, w, h) in faces:
        ...
        if 45 < confidence < 100:
            ...
            if recognition_count[customer_uid] >= REQUIRED_RECOGNITION_COUNT:
                ...
                sign_image_resized = cv2.resize(frame, (224, 224), interpolation=cv2.INTER_AREA)
                ...
                cv2.putText(frame, f"Sign: {class_name[2:]} ({confidence_score*100:.2f}%)", (10, 30), fontFace, 1, (0, 255, 0), 2)

                # Check if the sign is "OK" and the confidence is above threshold
                ok_sign_value = 1  # Default to 1 (incorrect)
                if class_name.strip() == "okay_sign" and confidence_score > 0.75:  # Adjust confidence threshold as needed
                    # ok_sign_value = 0
                    try:
                        c.execute("UPDATE customers SET ok_sign = ? WHERE customer_uid = ?", (ok_sign_value, customer_uid))
                        conn.commit() # Update the ok_sign column in the customers table

                        print(f"{customer_name}'s updated")
                    except sqlite3.Error as e:
                        print("SQLite error:", e)


    # Display the resulting frame
    cv2.imshow('Detecting Faces and Signs...', frame)
